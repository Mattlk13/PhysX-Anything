...
def normalize_scene(args) -> Tuple[float, Vector]:
    ...
    if args.rotate:
        scene.rotation_euler[2] += math.radians(180)
...
def main(arg):
    ...
    obj_files = glob.glob(os.path.join(arg.object, 'objs','*.obj'), recursive=True)
    ...
    # normalize scene
    scale, offset = normalize_scene(args)
    ...
    with open("views.json", "r", encoding="utf-8") as f:
        views = json.load(f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_folder', type=str, default='./dataset_new/renders', help='The path the output will be dumped to.')
    parser.add_argument('--resolution', type=int, default=1024, help='Resolution of the images.')
    parser.add_argument('--rotate', type=int, default=0, help='Resolution of the images.')
